...
export default function PortfolioSections({ pointerRef }) {
  ...
  return (
    <div className="pointer-events-none absolute inset-0 z-10 flex items-center justify-between p-6 sm:p-10 lg:p-14 select-none">
      {/* ============================================================ */}
      ...
      <div
        ...
        className="w-[340px] sm:w-[420px] lg:w-[460px] max-h-[82vh] rounded-3xl bg-white/80 backdrop-blur-2xl border border-neutral-900/[0.08] shadow-[0_25px_60px_-15px_rgba(0,0,0,0.08)] p-6 sm:p-7 flex flex-col justify-between overflow-y-auto"
      >
        <div>
          {/* Header & Section Tag */}
          <div className="flex items-center justify-between border-b border-neutral-900/[0.06] pb-3.5 mb-4">
            <div className="flex items-center gap-2">
              <Compass className="w-4 h-4 text-neutral-700" />
              <span className="text-[11px] font-semibold tracking-wider text-neutral-500 uppercase font-sans">
                Engineering Tracks
              </span>
            </div>
            <span className="text-[11px] font-medium text-neutral-400 font-sans">
              Select Focus
            </span>
          </div>
          ...
          {/* Track Selector */}
          <div className="grid grid-cols-2 gap-2.5 mt-4 p-1 rounded-2xl bg-neutral-100/80 border border-neutral-900/[0.05]">
            {/* Software Track */}
            <button
              onClick={() => setActiveTrack('web')}
              className={`flex items-center justify-center gap-2 py-2 px-3 rounded-xl font-sans text-xs font-semibold transition-all duration-200 ${
                activeTrack === 'web'
            ...
            {tracks[activeTrack].map((item, idx) => {
              ...
              return (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="group block p-3.5 rounded-2xl bg-white/70 hover:bg-white/95 border border-neutral-900/[0.06] hover:border-neutral-900/[0.12] transition-all duration-200 shadow-sm hover:shadow-md cursor-pointer"
                >
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-2.5">
                      <div className={`p-2 rounded-xl ${activeTrack === 'web' ? 'bg-blue-50 text-blue-600' : 'bg-rose-50 text-rose-600'}`}>
                      ...
                      <div>
                        ...
                        <span className="text-[10px] font-medium text-neutral-400 uppercase tracking-wider font-sans">
                          {item.category}
                        </span>
                      </div>
                    </div>
                  ...
                  <p className="mt-2 text-[11px] sm:text-xs text-neutral-600 font-sans leading-relaxed">
                    {item.desc}
                  </p>
                  ...
                  <div className="flex flex-wrap gap-1.5 mt-2.5">
                    {item.tags.map((t, i) => (
                      <span
                        key={i}
                        className={`px-2 py-0.5 text-[9px] font-medium font-sans rounded-md border ${
                          t === 'Published Paper'
        ...
        {/* Micro Footer Note */}
        <div className="mt-3 pt-2.5 border-t border-neutral-900/[0.06] flex items-center justify-between text-[10px] text-neutral-400 font-sans">
          <span>{activeTrack === 'web' ? 'Software & Systems Architecture' : 'Computer Vision & AI Systems'}</span>
      ...
      >
        <div>
          ...
          <div className="flex items-center justify-between border-b border-neutral-900/[0.06] pb-3.5 mb-4">
            ...
            <div className="flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-emerald-50 border border-emerald-200/60">
              ...
              <span className="text-[10px] font-semibold text-emerald-700 font-sans">Open to Collaboration</span>
            </div>
          </div>
          ...
          <p className="mt-1.5 text-xs sm:text-sm text-neutral-600 font-sans leading-relaxed">
            Researcher and software engineer specializing in <strong className="text-neutral-900 font-medium">Computer Vision & AI</strong> alongside <strong className="text-neutral-900 font-medium">Interactive Web & Systems Engineering</strong>.
          </p>

          {/* Academic & Engineering Pillars */}
          <div className="mt-4 space-y-2">
            <div className="p-3 rounded-xl bg-neutral-50 border border-neutral-900/[0.04] flex items-center gap-3">
              <div className="p-2 rounded-lg bg-rose-100/60 text-rose-700">
                <Cpu className="w-4 h-4" />
              </div>
              <div>
                <h4 className="text-xs font-semibold text-neutral-900 font-sans">Computer Vision & AI</h4>
                <p className="text-[10px] text-neutral-500 font-sans">Physics-informed neural networks (PINNs), temporal risk anticipation, and biomechanical vision pipelines.</p>
              </div>
            </div>
            ...
            <div className="p-3 rounded-xl bg-neutral-50 border border-neutral-900/[0.04] flex items-center gap-3">
              ...
              <div>
                <h4 className="text-xs font-semibold text-neutral-900 font-sans">Full-Stack & Native Systems</h4>
                <p className="text-[10px] text-neutral-500 font-sans">Scalable cloud architectures, native macOS tooling, and immersive 3D spatial web interfaces.</p>
              </div>
            </div>
          </div>

          {/* Direct Research & Code Links Strip */}
          <div className="mt-5 pt-4 border-t border-neutral-900/[0.06]">
            <span className="text-[10px] font-semibold text-neutral-400 uppercase tracking-wider font-sans block mb-2.5">
            ...
            <div className="grid grid-cols-5 gap-1.5">
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="flex flex-col items-center justify-center py-2 px-1 rounded-xl bg-neutral-100/80 hover:bg-rose-600 hover:text-white text-neutral-700 font-sans text-[11px] font-medium transition-all duration-200 text-center gap-1"
              >
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="flex flex-col items-center justify-center py-2 px-1 rounded-xl bg-neutral-100/80 hover:bg-neutral-900 hover:text-white text-neutral-700 font-sans text-[11px] font-medium transition-all duration-200 text-center gap-1"
              >
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="flex flex-col items-center justify-center py-2 px-1 rounded-xl bg-neutral-100/80 hover:bg-blue-600 hover:text-white text-neutral-700 font-sans text-[11px] font-medium transition-all duration-200 text-center gap-1"
              >
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="flex flex-col items-center justify-center py-2 px-1 rounded-xl bg-neutral-100/80 hover:bg-emerald-600 hover:text-white text-neutral-700 font-sans text-[11px] font-medium transition-all duration-200 text-center gap-1"
              >
        ...
        <div className="mt-3 pt-2.5 border-t border-neutral-900/[0.06] flex items-center justify-between text-[10px] text-neutral-400 font-sans">
          <span>Scroll down for deep dive</span>
